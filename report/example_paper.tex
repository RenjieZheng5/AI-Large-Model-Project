...
\begin{enumerate}
    ...
    \item Why can Basic RAG be faster than direct LLM answering even though it adds retrieval?
\end{enumerate}
...
To explain the surprising latency result that Basic RAG is faster than Direct, we run an additional diagnostic experiment on the same 30 questions. This experiment compares only Direct and Basic RAG and records prompt tokens, completion tokens, retrieval latency, generation latency, total latency, and generation throughput from the vLLM API response. This separates the cost of retrieval from the cost of autoregressive answer generation.
...
\subsection{Latency Trade-off and Diagnostic Analysis}
...
The diagnostic experiment explains why Basic RAG can even be faster than Direct. Table~\ref{tab:latency_diagnostic} shows that Basic RAG spends 0.741s on retrieval on average. Its prompt is longer because it includes retrieved evidence, but its answer is much shorter: 56.13 completion tokens compared with 180.53 for Direct. Since autoregressive decoding cost scales mainly with the number of generated tokens, this output-length reduction dominates the retrieval overhead. The generation throughput is similar for the two settings, with 20.58 tokens/s for Direct and 18.89 tokens/s for Basic RAG, so the result should not be interpreted as the model becoming faster per token. Instead, the retrieved evidence and stricter grounded-answer instruction reduce open-ended explanation and hallucinated elaboration. The correlation between generation latency and completion tokens is also almost perfect in this diagnostic run: 1.0000 for Direct and 0.9998 for Basic RAG.

\begin{table}[t]
\centering
\caption{Latency diagnostic for Direct and Basic RAG on the same 30 questions.}
\label{tab:latency_diagnostic}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lrrrrr}
\toprule
System & Input Tok. & Output Tok. & Retrieval & Generation & Total \\
\midrule
Direct & 51.80 & 180.53 & 0.000s & 8.757s & 8.757s \\
Basic RAG & 580.73 & 56.13 & 0.741s & 2.842s & 3.583s \\
\bottomrule
\end{tabular}
}
\end{table}
...
\begin{figure}[t]
\centering
\safeincludegraphics[width=\columnwidth]{figures/rag_latency_diagnostic_breakdown.png}
\caption{Latency decomposition for Direct and Basic RAG. Retrieval overhead is small; the main difference is generation time.}
\label{fig:latency_breakdown}
\end{figure}

\begin{figure}[t]
\centering
\safeincludegraphics[width=\columnwidth]{figures/rag_latency_vs_output_tokens.png}
\caption{Generation latency versus completion tokens. The near-linear pattern shows that shorter RAG answers are the main reason for lower latency.}
\label{fig:latency_tokens}
\end{figure}
...
The experiments support four findings. First, RAG greatly improves reliability for local knowledge: direct answering is weak because campus details are not reliably stored in model parameters. Second, reranking improves evidence ordering and therefore improves downstream correctness. Third, query rewriting improves retrieval recall but adds latency. Fourth, Basic RAG can be faster than direct answering when the retrieved context constrains output length. Better retrieval is necessary but not always sufficient: when retrieved context is incomplete or conflicting, the generator may still refuse or produce an incomplete answer.
...
This project implements and evaluates a complete RAG system for SUSTech campus knowledge question answering. Compared with direct Qwen3-32B answering, RAG improves answer correctness from 36.67\% to at least 80.00\% and reduces hallucination from 66.67\% to 0.00\%. Reranking further improves correctness to 86.67\%. LLM query rewriting achieves the best retrieval quality, reaching 96.67\% Recall@5 and 95.86\% nDCG@5. The latency diagnostic further shows that Basic RAG is faster because it reduces average completion length from 180.53 to 56.13 tokens, more than compensating for retrieval overhead. Overall, the system demonstrates that retrieval augmentation is highly effective for institution-specific question answering, especially when factual accuracy, grounding, source traceability, and concise response generation are required.
