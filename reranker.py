...
class BGEReranker:
    ...
    def build_doc_text(self, item: Dict[str, Any]) -> str:
        """Build a readable candidate text for cross-encoder relevance scoring."""
        ...
        text = f"问题：{question}\n答案：{answer}" if question or answer else index_text
        if category:
            text += f"\n类别：{category}"
        if source_title:
            text += f"\n来源标题：{source_title}"
    ...
    def rerank(
        ...
        pairs = [[query, self.build_doc_text(item)] for item in candidates]
